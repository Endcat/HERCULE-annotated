...
class HERCULE:
        ...
        for i in range(node_amt):
            ...
            for j in range(i + 1, node_amt):
                ...
                relation = self.generate_relation(
                    node1, node2, TIME_DELTA, DURATION_DELTA)
    ...
    def train_alpha(self, datafile_x, datafile_y):
        ...
        curr_alpha, curr_loss = sess.run(
            [alpha, loss], {x: x_train, y: y_train})
    ...
    def detect_community(self):
        ...
        partition = community.partition_at_level(
...
def main():
        # program entry
    hercule = HERCULE()  # 实例化hercule
    datadir = os.path.dirname(os.path.dirname(os.path.abspath(
        __file__))) + os.path.sep + 'data' + os.path.sep + 'sjtu_flow' + os.path.sep + 'mini'
    """
    ...
    # 读取文件 原log数据取自stju流量数据
    test_file_list = [datadir + os.path.sep + 'flow_test1.log',
                      datadir + os.path.sep + 'resp_test1.log']
    ...
    # evaluation
    corrected_file_list = [datadir + os.path.sep + 'flow_corrected1.log',
                           datadir + os.path.sep + 'resp_corrected1.log']
